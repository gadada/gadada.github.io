...
每个`f[i][j]`的值由其“左上的元素`f[i-1][j - w[i]]`”和“正上的元素`f[i-1][j]`”决定，这个记忆型递归是从上往下，从左往右的

为了节省空间，这里可以引入滚动数组，因为计算`f[i][j]`只需要其左上或正上的元素，从右至左计算的话就可以将`f[i][j]`的值覆盖到`f[i-1][j]`，因为此时`f[i-1][j]`已经没有作用了，从而化简成一维数组，递推式为`f[j] = max(f[j], f[j - w[i]] + d[i])`
