...
- 当`i*j < k`时，无法完成分割，这里用`memset(dp, 0x0f, sizeof(dp))`将其初始化即可
...
int Dfs(int w,int h,int m) //切m刀
{
	...
	for(int i = 1; i < w; ++i) { //枚举第一刀的竖着切的各种切法
		for(int k = 1; k < m; ++k) { //枚举左半边接下来被切成k+1块的各种切法
			int m1 = Dfs(i,h,k);
	...
	for(int i = 1; i < h; ++i) {  //枚举第一刀的横着切的各种切法
		int m1,m2;
		for(int k = 1; k < m; ++k) { //枚举上半边接下来被切成k+1块的各种切法 1
			m1 = Dfs(w,i,k);
			m2 = Dfs(w,h-i,m-k);
			mx = min(mx,max(m1,m2));
		}
...
至此dynamic program的各种类型都覆盖到了，比递归要难上不少，就像钻石排列dynamic amoled吊打周冬雨排列amoled，你大爷还是你大爷