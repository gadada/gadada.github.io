...
该题要从某一点出发找能够遍历所有点的通路，因此要用dfs，另外要注意马走日

此外，输入时打印其路径，行用阿拉伯数字`1,2...`，列用`A,B...`，打印时要遵循字典序
...
int Isway(int i, int j)
{
	if (i < 0 || i >= p || j < 0 || j >= q) return 0;
	if (visit[i][j])return 0;
	return 1;
}
int dfs(struct node *W, int P)
{
	if (W->cnt == t) {
		result = P;
		return 1;
...
void printway(int r)
{
	if (Q[r].last != -1) printway(Q[r].last);
	printf("%c%c", Q[r].j + 'A', Q[r].i + '1');
}
int main()
{
	//freopen("E:\\IDMdowanload\\in.txt", "r", stdin);
...
```
以上过程为递归开了个巨大的数组，其实可以在dfs递归过程中设置一个数组`way`记录路径，而不用“指针”`last`记录上一个节点
